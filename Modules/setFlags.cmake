...
function(set_LIBRARY_UTIL)
    ...
    set (NDATE_Fortran_FLAGS "${HOST_FLAG} ${FP_MODEL_SOURCE} ${FTZ} ${BYTERECLEN} ${BIG_ENDIAN} -heap-arrays  -DCOMMCODE -DLINUX -DUPPLITTLEENDIAN ${FOPT3} -Wl,-noinhibit-exec" CACHE INTERNAL "" )
    set(NCDIAG_Fortran_FLAGS "${FREEFORM} ${BYTERECLEN} ${BIG_ENDIAN}" CACHE INTERNAL "" )
...
function(set_LIBRARY_UTIL_Debug)
    ...
    set(NDATE_Fortran_FLAGS "${HOST_FLAG} ${FP_MODEL_SOURCE} ${FTZ} ${BYTERECLEN} ${BIG_ENDIAN} -heap-arrays  -DCOMMCODE -DLINUX -DUPPLITTLEENDIAN ${DEBINFO} -Wl,-noinhibit-exec" CACHE INTERNAL "" )
...
function(set_GSI_ENKF)
    ...
    set(GSI_Fortran_FLAGS "${GSI_Platform_FLAGS} ${GSDCLOUDOPT}" CACHE INTERNAL "Full GSI Fortran FLAGS" )
...
function (set_GSI_ENKF_Debug)
    set(GSI_Fortran_FLAGS "-DPOUND_FOR_STRINGIFY ${FOPT0} ${FP_MODEL_SOURCE} ${BIG_ENDIAN} ${BYTERECLEN} ${IMPLICITNONE} -${MC_MODEL_MEDIUM} ${SHARED}-intel ${DEBINFO} ${TRACEBACK} ${STACKLOCAL} ${CHECKALL},noarg_temp_created ${FP_STACK_CHECK} ${WARNALL},nointerfaces ${BIG_ENDIAN} ${IMPLICITNONE} -D_REAL8_ ${OpenMP_Fortran_FLAGS} ${MPI_Fortran_COMPILE_FLAGS} ${GSDCLOUDOPT}" CACHE INTERNAL "")
    set(ENKF_Fortran_FLAGS "${FOPT0} ${HOST_FLAG} ${WARNALL} ${IMPLICITNONE} ${TRACEBACK} ${DEBINFO} ${DEBUG} ${CHECKALL},noarg_temp_created ${FP_MODEL_STRICT} ${BIG_ENDIAN} ${BYTERECLEN} -D_REAL8_  ${OpenMP_Fortran_FLAGS} ${GSDCLOUDOPT}" CACHE INTERNAL "")
